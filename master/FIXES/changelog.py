...
def showChangeLog(env):
    text = ''
    if env.lower() == 'dev':
        url = 'https://github.com/mafiatv/MafiaTV/commits/master'
        env = 'Dev'
        ...
        if selfAddon.getSetting("autosource") == "false":
            if selfAddon.getSetting("updateoption") == "gitupdate1":
                ...
                url = 'https://github.com/mafiatv/AutoUpdate/commits/master'
	else:
	    import time
	    url='https://bitbucket.org/!api/2.0/repositories/mafiatv/bitautoupdate1/commits?'+ str(time.time())
	env = ''
